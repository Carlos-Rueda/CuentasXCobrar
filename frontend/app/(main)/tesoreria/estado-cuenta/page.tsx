...
export default function EstadoCuentaPage() {
  ...
  const exportarAExcel = async () => {
    ...
    todosLosMovimientos.forEach((t) => {
      ...
      const refLimpia = (t.referencia || "")
        .replace(/;/g, ",")
        .replace(/\n/g, " ");
      const descLimpia = (t.descripcion || "")
        .replace(/;/g, ",")
        .replace(/\n/g, " ");
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-4 space-y-4">
            <h3 className="text-xs font-bold text-slate-455 uppercase tracking-wider px-1">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between z-10">
                      <div>
                        <span
                          className={`text-[9px] uppercase px-2 py-0.5 rounded-full font-extrabold tracking-wider ${
                            isSelected
                              ? "bg-red-105 text-[var(--utn-red)]"
                              : "bg-slate-100 text-slate-500"
                          }`}
                        >
                          {rep.tipoCuenta || "Ahorros"}
                        </span>
                        <h4 className="font-black text-base mt-1.5 tracking-tight">
                          {rep.nombreBanco}
                        </h4>
                      ...
                      <Landmark
                        className={`w-7 h-7 ${isSelected ? "text-[var(--utn-red)]" : "text-slate-300"}`}
                      />
                    </div>
                    ...
                    <div className="mt-2 flex items-baseline justify-between z-10 border-t border-slate-200/50 pt-2">
                      ...
                      <span
                        className={`text-lg font-black ${isSelected ? "text-[var(--utn-red)]" : "text-slate-800"}`}
                      >
          ...
          <div className="lg:col-span-8 space-y-6">
            {selectedReport ? (
                ...
                <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-6">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <h2 className="text-2xl font-black text-slate-855 mt-0.5">
                        {selectedReport.nombreBanco}
                      </h2>
                      <p className="text-xs text-slate-550 mt-0.5">
                        Titular:{" "}
                        <span className="font-semibold text-slate-655">
                          {selectedReport.nombreCuenta || "UTN Principal"}
                        </span>{" "}
                        — Tipo:{" "}
                        <span className="font-semibold text-slate-655 capitalize">
                          {selectedReport.tipoCuenta || "Ahorros"}
                        </span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-100 pt-6">
                    <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4 flex flex-col justify-between">
                      ...
                      <p className="text-lg font-black text-emerald-755 mt-2">
                        {formatCurrency(selectedReport.saldo_facturacion)}
                      </p>
                    </div>
                    ...
                    {localFechaInicio || localFechaFin ? (
                      ...
                      <div className="bg-red-50/50 border border-red-100 rounded-2xl p-4 flex flex-col justify-between">
                        <span className="text-[10px] font-bold text-[var(--utn-red)] uppercase tracking-wider">
                          Total Consolidado
                        </span>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 border-t border-slate-100 pt-6">
                    <div className="bg-slate-50/70 border border-slate-200/50 rounded-2xl p-3.5">
                      ...
                      <p className="text-sm font-black text-slate-800 mt-1">
                        {formatCurrency(sumRecaudadoCxc)}
                      </p>
                    </div>
                    <div className="bg-slate-50/70 border border-slate-200/50 rounded-2xl p-3.5">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block">
                      ...
                      <p className="text-sm font-black text-emerald-655 mt-1">
                        {formatCurrency(sumTransfRecibidas)}
                      </p>
                    </div>
                    <div className="bg-slate-50/70 border border-slate-200/50 rounded-2xl p-3.5">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block">
                      ...
                      <p className="text-sm font-black text-[var(--utn-red)] mt-1">
                        {formatCurrency(sumTransfEnviadas)}
                      </p>
                    </div>
                    <div className="bg-slate-50/70 border border-slate-200/50 rounded-2xl p-3.5">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block">
                      ...
                      <p className="text-sm font-black text-slate-800 mt-1">
                        {formatCurrency(sumPagosExternos)}
                      </p>
                    </div>
                    <div className="bg-slate-50/70 border border-slate-200/50 rounded-2xl p-3.5">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block">
                      ...
                      <p className="text-sm font-black text-[var(--utn-red)] mt-1">
                        {formatCurrency(sumComprasEgresos)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden min-h-[300px]">
                  ...
                  {activeTab === "all" && (
                    <div className="p-4 space-y-4">
                      <div className="flex justify-between items-center px-2">
                        <span className="text-xs font-bold text-slate-455 uppercase tracking-wider">
                          Historial Consolidado del Período
                        </span>
                          ...
                          <table className="min-w-full text-xs text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 font-medium text-slate-600">
                              {todosLosMovimientos.map((t) => {
                                const isIncome =
                                  t.tipo === "ingreso" ||
                                  t.referencia.includes("Recibida");
                                return (
                                  <tr
                                    key={t.id}
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3.5">
                                      ...
                                      <div className="text-[10px] text-slate-450 max-w-xs truncate">
                                        {t.descripcion}
                                      </div>
                                    </td>
                                    <td
                  ...
                  {activeTab === "purchases" && (
                    <div className="p-4 space-y-4">
                      <div className="flex justify-between items-center px-2">
                        <span className="text-xs font-bold text-slate-455 uppercase tracking-wider">
                          Egresos por Compras (Proveedores)
                        </span>
                        <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-bold">
                      ...
                      {comprasFiltradas.length === 0 ? (
                        <div className="text-center py-16 text-slate-500">
                          Sin egresos por compras en este período.
                        </div>
                      ) : (
                        <div className="overflow-x-auto">
                          <table className="min-w-full text-xs text-left">
                  ...
                  {activeTab === "cxc" && (
                    <div className="p-4 space-y-4">
                      ...
                      {pagosRecaudadosCxcFiltradas.length === 0 ? (
                        <div className="text-center py-16 text-slate-555">
                          Sin cobros de CXC en este período.
                        </div>
                      ) : (
                        <div className="overflow-x-auto">
                          <table className="min-w-full text-xs text-left">
                  ...
                  {activeTab === "billing" && (
                    <div className="p-6 space-y-6">
                      <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                        <span className="text-xs font-bold text-slate-455 uppercase tracking-wider">
                          Recaudación Facturación Externa
                        </span>
                      </div>
                      ...
                      <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-6 space-y-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <h4 className="font-extrabold text-slate-855 text-sm">
                              Integración Módulo Facturación (GraphQL)
                            </h4>
                            <p className="text-xs text-slate-550 mt-1">
                              Los cobros procesados por facturación en línea
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center text-slate-455 shadow-sm">
                Selecciona una cuenta del listado izquierdo para ver los
                detalles.
              </div>
            )}
          </div>
