...
export class FacturasController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Datos de la factura encontrada.',
    type: FacturaDto,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Listado de facturas con estado PENDIENTE.',
    type: [FacturaDto],
  })
  async getPendientes(
    @Param('clienteId') clienteId: string,
  ): Promise<FacturaDto[]> {
    return await this.facturasService.findFacturasPendientesByCliente(
      clienteId,
    );
  }
}
