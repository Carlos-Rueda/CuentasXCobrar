...
export class AuditoriaService implements OnModuleInit {
  ...
  private readonly authUrl =
    'https://712286fsib.execute-api.us-east-1.amazonaws.com/default/api-auth-central';
  ...
  onModuleInit() {
    try {
      ...
      this.logger.log(
        `Archivo Protobuf de auditoría cargado correctamente (Sync) desde: ${protoPath}`,
      );
    } catch (error) {
      this.logger.error(
        'Error cargando el archivo auditoria.proto de forma síncrona',
        error.message,
      );
    }
  }
  ...
  private async authenticate(): Promise<string> {
    ...
    try {
      ...
      if (response.data?.success && response.data?.token) {
        ...
        this.logger.log(
          'Autenticación con el Módulo de Seguridad para Auditoría exitosa.',
        );
        return this.jwtToken;
      } else {
      ...
      this.logger.error(
        'Falló la autenticación para auditoría',
  ...
  public async registrarTraza(
    ...
    try {
      ...
      if (!tokenToUse || tokenToUse.trim() === '') {
        this.logger.warn(
          'Token JWT no provisto ni disponible por autenticación del módulo. Se omite el envío de la pista de auditoría.',
        );
        return;
      }
      ...
      this.logger.log(
        `Pista de auditoría centralizada enviada con éxito (Status: ${response.status})`,
      );
    } catch (error: any) {
      ...
      console.error(
        'Error enviando la pista de auditoría centralizada:',
        errorMsg,
      );
